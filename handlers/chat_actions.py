...
# getting commands
@dp.message_handler(is_admin = True, commands=['start', 'help','помощь'], commands_prefix = '!/.?%')
...
@dp.message_handler(commands = ['admins','админы'], commands_prefix = '!/.?%')
...
# set group photo
@dp.message_handler(is_admin = True, commands = ['photo','фото'], commands_prefix='!/')
...
# delete group photo
@dp.message_handler(is_admin = True, commands = ['delphoto','делфото'], commands_prefix ='!/')
...
# get invite url
@dp.message_handler(is_admin = True, commands=['url','ссылка'], commands_prefix = '!/.?%')
...
#permanent mute
@dp.message_handler(is_admin = True, commands=['permmute','перммут'], commands_prefix = '!/.?%')
...
# permanent ban
@dp.message_handler(is_admin = True, commands=['permban','пермбан'], commands_prefix = '!/.?%')
async def ban(message: types.Message):
	if not message.reply_to_message:
		await message.reply('Эта команда должна быть ответом на сообщение.')
		return
...
# set administrator
@dp.message_handler(is_owner = True, commands = ['setadmin','сетадмин'], commands_prefix = '!/.?%')
...
@dp.message_handler(commands=['setprefix','префикс'], commands_prefix='!/')
...
@dp.message_handler(is_owner = True, commands=['collectstats','сбор'], commands_prefix = '!/.?%')
...
# add user to db
@dp.message_handler(is_admin = True, commands=['dbadd','дб'], commands_prefix = '!/.?%')
...
@dp.message_handler(commands=['top_user','топюзер'], commands_prefix = '!/.?%')
